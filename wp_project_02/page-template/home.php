<?php
/**
 * Template Name: Home Layout Template
 */
get_header(); ?>

<!-- Banner Section Start -->
<section class="banner-section position-relative filter-banner has-overlay">
    <div class="bg-media" style="background-image: url(/getty-images-_-zbpTrVZTo-unsplash.webp)"></div>
    <div class="container">
        <div class="col-12">
            <div class="content-wrap text-center">
                <h1>Discover The Best Jobs In Travel Healthcare</h1>
                <h5>Find exciting travel nursing, allied health, patient support & non-clinical jobs today!</h5>
            </div>
            
            <div class="banner-filter position-relative">
                <div class="row align-items-center banner-filter-header">
                    <div class="col-12 col-md-6 banner-filter-content">
                        <p><strong>500+ amazing reviews</strong> and counting</p>
                    </div>
                    <div class="col-12 col-md-6 banner-filter-images">
                        <div>
                            <a href="#">
                                <img src="/vivan-review.png" alt="">
                            </a>
                        </div>
                        <div>
                            <a href="#">
                                <img src="/google-review-1.png" alt="">
                            </a>
                        </div>
                        <div>
                            <a href="#">
                                <img src="/indeed-review.png" alt="">
                            </a>
                        </div>
                        <div>
                            <a href="#">
                                <img src="/winner-2024.svg" alt="">
                            </a>
                        </div>
                    </div>
                </div>
                <div class="looking-filter">
                    <div class="filter-header d-flex justify-content-between align-items-center">
                        <h2 class="h3 mb-0">What are you looking for?</h2>
                        <div class="recruiting-link">
                            <p class="mb-0">Recruiting? <a href="#" class="text-primary text-decoration-none">Click Here</a></p>
                        </div>
                    </div>
                    <form class="filter-form" action="" method="GET">
                        <div class="row">
                            <div class="col-12 col-lg-10">
                                <div class="row g-3">
                                    <div class="col-12 col-md-6 col-lg-3">
                                        <select name="job_title" class="form-select py-3">
                                            <option value="">Job Title</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-6 col-lg-3">
                                        <select name="speciality" class="form-select py-3">
                                            <option value="">Speciality</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-6 col-lg-3">
                                        <select name="location" class="form-select py-3">
                                            <option value="">Location</option>
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-6 col-lg-3">
                                        <select name="employment_type" class="form-select py-3">
                                            <option value="">Employment Type</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-lg-2">
                                <button type="submit" class="btn btn-primary w-100 py-3 d-flex align-items-center justify-content-center gap-2">
                                    Search
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
                                </button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- Banner Section End -->


<section class="popular-jobs space-y">
    <div class="container">
        <div class="section-header mb-5m text-center" data-aos="fade-up">
            <div class="inner-wrap">
                <p><strong>Positions</strong></p>
                <h2>Our Most Popular Jobs</h2>
                <a href="#" class="cta-link"><span>View All Roles</span> <i class="fa-light fa-arrow-right-long"></i></a>
                <div class="slider-nav d-flex gap-4">
                    <button class="popular-jobs-prev" aria-label="Previous">
                        <i class="fa-regular fa-arrow-left"></i>
                    </button>
                    <button class="popular-jobs-next" aria-label="Next">
                        <i class="fa-regular fa-arrow-right"></i>
                    </button>
                </div>
            </div>
        </div>
        <div class="popular-jobs-slider" data-aos="fade-up" data-aos-delay="100">
            <div class="popular-jobs-item">
                <div class="card-popular-job" style="background-image: url('/Popular-Jobs-1.webp');">
                    <div class="card-content">
                        <h3>Travel Nursing</h3>
                        <ul>
                            <li>list content 1</li>
                            <li>list content 2</li>
                            <li>list content 3</li>
                        </ul>
                        <a href="#" class="btn btn-primary">View Roles</a>
                    </div>
                </div>
            </div>
            <div class="popular-jobs-item">
                <div class="card-popular-job" style="background-image: url('/Popular-Jobs-2.webp');">
                    <div class="card-content">
                        <h3>Allied Health</h3>
                        <ul>
                            <li>list content 1</li>
                            <li>list content 2</li>
                            <li>list content 3</li>
                        </ul>
                        <a href="#" class="btn btn-primary">View Roles</a>
                    </div>
                </div>
            </div>
            <div class="popular-jobs-item">
                <div class="card-popular-job" style="background-image: url('/Popular-Jobs-3.webp');">
                    <div class="card-content">
                        <h3>Non-Clinical</h3>
                        <ul>
                            <li>list content 1</li>
                            <li>list content 2</li>
                            <li>list content 3</li>
                        </ul>
                        <a href="#" class="btn btn-primary">View Roles</a>
                    </div>
                </div>
            </div>
            <div class="popular-jobs-item">
                <div class="card-popular-job" style="background-image: url('/Popular-Jobs-4.webp');">
                    <div class="card-content">
                        <h3>Permanent</h3>
                        <ul>
                            <li>list content 1</li>
                            <li>list content 2</li>
                            <li>list content 3</li>
                        </ul>
                        <a href="#" class="btn btn-primary">View Roles</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>


<section class="how-works-sec space-y">
    <div class="container">
        
        <div class="row justify-content-between">
            <div class="col-lg-6 col-md-6 col-sm-12 how-works-left-col">
                <div class="inner-how-works-col animated fadeInUp" data-class="fadeInUp">
                    <div class="how-works-img">
                        <img src="/how-it-work-left-image.webp" alt="">
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12 how-works-right-col">
                <div class="inner-how-works-right-col">
                    <div class="heading-wrap">
                        <h6>How it Works</h6>
                        <h2 class="h1">The Smartest <br>Way To Travel</h2>
                    </div>
                    <div class="how-works-right-list">
                        <div class="how-works-item animated delay-300 fadeInUp" data-class="fadeInUp">
                            <div class="how-works-item-row c-row">
                                <div class="how-works-item-col how-works-item-left-col">
                                    <span>01</span>
                                </div>
                                <div class="how-works-item-col how-works-item-right-col">
                                    <h3>Sign Up</h3>
                                    <p>Get started in seconds with our fast and hassle-free application.</p>
                                </div>
                            </div>
                        </div>
                        <div class="how-works-item animated delay-300" data-class="fadeInUp">
                            <div class="how-works-item-row c-row">
                                <div class="how-works-item-col how-works-item-left-col">
                                    <span>02</span>
                                </div>
                                <div class="how-works-item-col how-works-item-right-col">
                                    <h3>Connect</h3>
                                    <p>Meet your dedicated recruiter, ready to guide you every step of the way.</p>
                                </div>
                            </div>
                        </div>
                        <div class="how-works-item animated delay-300" data-class="fadeInUp">
                            <div class="how-works-item-row c-row">
                                <div class="how-works-item-col how-works-item-left-col">
                                    <span>03</span>
                                </div>
                                <div class="how-works-item-col how-works-item-right-col">
                                    <h3>Find Your Fit</h3>
                                    <p>Browse and apply for top healthcare roles tailored to your skills and goals.</p>
                                </div>
                            </div>
                        </div>
                        <div class="how-works-item animated delay-300" data-class="fadeInUp">
                            <div class="how-works-item-row c-row">
                                <div class="how-works-item-col how-works-item-left-col">
                                    <span>04</span>
                                </div>
                                <div class="how-works-item-col how-works-item-right-col">
                                    <h3>Start Your Journey</h3>
                                    <p>Step into your dream job and embrace an exciting new adventure.</p>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>


<!-- Perks Section Start -->
<section class="perks-section space-y">
    <div class="container">
        <div class="section-header mb-5m text-center" data-aos="fade-up">
            <div class="inner-wrap col-12 col-lg-6 mx-auto">
                <p><strong>Perks</strong></p>
                <h2 class="mb-0">Perks You'll Love</h2>
                <h5>We make your travel experience smoother, more rewarding, and stress-free</h5>
            </div>
        </div>
        <div class="perks-grid-wrap">
            <div class="row g-5">
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Better-Pay.svg" alt="Competitive Pay" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Better Pay</h4>
                            <p>We’re proud to offer some of the best gross pay packages in travel healthcare</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="200">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Certificate-License-Reimbursements.svg" alt="Comprehensive Benefits" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Certificate & License Reimbursements</h4>
                            <p>Licensing fees can add up fast—we take care of them, so you don’t have to.</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="300">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Holiday-Pay.svg" alt="Flexible Scheduling" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Holiday Pay</h4>
                            <p>Enjoy exploring your new surroundings, fully paid and well deserved</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="400">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Better-Pay.svg" alt="24/7 Support" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Health Insurance</h4>
                            <p>Travel with peace of mind that you are covered from any unexpected surprises</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="500">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Weekly-Pay.svg" alt="Career Growth" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Weekly Pay</h4>
                            <p>Fast, reliable, and on time—because waiting for your paycheck isn’t fun!</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="600">
                    <div class="perk-card text-center">
                        <div class="perk-icon">
                            <img src="/Referral-Bonus.svg" alt="Prime Locations" class="svg-convert">
                        </div>
                        <div class="perk-content">
                            <h4>Referral Bonus</h4>
                            <p>Earn $500 for every travel healthcare professional you refer. Easy money! (T&Cs apply.)</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- Perks Section End -->

<!-- Reviews Section -->
<section class="reviews-section space-y">
    <div class="container">
        <div class="reviews-wrap position-relative">
            <div class="floating-reviews">
                <div class="review-avatar review-pos-1" data-aos="fade-right" data-aos-delay="100">
                    <img src="/getty-images-_-zbpTrVZTo-unsplash-1.png" alt="Review">
                </div>
                 <div class="review-avatar review-pos-2" data-aos="fade-right" data-aos-delay="100">
                    <img src="/chirsty.png" alt="Review">
                </div>
                <div class="review-avatar review-pos-3" data-aos="fade-right" data-aos-delay="100">
                    <img src="/lable-image-3.png" alt="Review">
                </div>
                 <div class="review-avatar review-pos-4" data-aos="fade-right" data-aos-delay="100">
                    <img src="/chris-leipelt.png" alt="Review">
                </div>
                <div class="review-avatar review-pos-5" data-aos="fade-right" data-aos-delay="100">
                    <img src="/lable-image-2.png" alt="Review">
                </div>
                 <div class="review-avatar review-pos-6" data-aos="fade-right" data-aos-delay="100">
                    <img src="/usman-yousaf.png" alt="Review">
                </div>
                <div class="review-avatar review-pos-7" data-aos="fade-right" data-aos-delay="100">
                    <img src="/lable-image-1.png" alt="Review">
                </div>
                <div class="review-avatar review-pos-8" data-aos="fade-right" data-aos-delay="100">
                    <img src="/getty-images-KaL3LbAmPPI-unsplash-1.png" alt="Review">
                </div>
            </div>
            
            <div class="reviews-content text-center" data-aos="fade-up">
                <p class="text-primary"><strong>Reviews</strong></p>
                <h2 class="mb-4">Recommended By<br>Hundreds Of Happy<br>Healthcare Professionals</h2>
                <div class="google-rating d-inline-flex align-items-center gap-2 mb-0">
                    <img src="/google-review.svg" alt="Google" width="20">
                </div>
            </div>
        </div>
    </div>
</section>
<!-- Reviews Section End -->



<!-- jobs-sections start -->
<section class="jobs-sections space-y">
    <div class="section-header mb-5m text-center" data-aos="fade-up">
        <div class="container">
            <div class="inner-wrap">
                <p><strong>Healthcare Jobs</strong></p>
                <h2>Check Out Our Latest Jobs</h2>
                <a href="#" class="cta-link"><span>View All Roles</span> <i class="fa-light fa-arrow-right-long"></i></a>
                <div class="slider-nav d-flex gap-4">
                    <button class="jobs-prev" aria-label="Previous">
                        <i class="fa-regular fa-arrow-left"></i>
                    </button>
                    <button class="jobs-next" aria-label="Next">
                        <i class="fa-regular fa-arrow-right"></i>
                    </button>
                </div>
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="jobs-slider" data-aos="fade-up" data-aos-delay="100">
            <div class="carousel-cell">
                <div class="card-jobs">
                    <div class="card-badge">New</div>
                    <h3 class="job-title">Travel Nurse/Registered Nurse (RN) – MedSurg/Telemetry</h3>
                    <ul class="job-info">
                        <li>
                            <img src="/job-location-icon.svg" alt="">
                            <span><strong>Location:</strong> Charlotte, North Carolina, USA</span>
                        </li>
                        <li>
                            <img src="/job-Specialty-icon.svg" alt="">
                            <span><strong>Specialty:</strong> Health Care</span>
                        </li>
                        <li>
                            <img src="/job-type-icon.svg" alt="">
                            <span><strong>Type:</strong> Contract</span>
                        </li>
                        <li>
                            <img src="/job-Schedule-icon.svg" alt="">
                            <span><strong>Schedule:</strong> Lorem ipsum</span>
                        </li>
                        <li>
                            <img src="/job-time-icon.svg" alt="">
                            <span><strong>Time:</strong> Lorem ipsum</span>
                        </li>
                    </ul>
                    <div class="card-actions">
                        <a href="#" class="btn btn-primary"><span>Read More</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-secondary"><span>Apply Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <button type="button" class="btn-share">
                            <i class="fa-light fa-share-nodes"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="carousel-cell">
                <div class="card-jobs">
                    <div class="card-badge">New</div>
                    <h3 class="job-title">Travel Nurse/Registered Nurse (RN) – MedSurg/Telemetry</h3>
                    <ul class="job-info">
                        <li>
                            <img src="/job-location-icon.svg" alt="">
                            <span><strong>Location:</strong> Charlotte, North Carolina, USA</span>
                        </li>
                        <li>
                            <img src="/job-Specialty-icon.svg" alt="">
                            <span><strong>Specialty:</strong> Health Care</span>
                        </li>
                        <li>
                            <img src="/job-type-icon.svg" alt="">
                            <span><strong>Type:</strong> Contract</span>
                        </li>
                        <li>
                            <img src="/job-Schedule-icon.svg" alt="">
                            <span><strong>Schedule:</strong> Lorem ipsum</span>
                        </li>
                        <li>
                            <img src="/job-time-icon.svg" alt="">
                            <span><strong>Time:</strong> Lorem ipsum</span>
                        </li>
                    </ul>
                    <div class="card-actions">
                        <a href="#" class="btn btn-primary"><span>Read More</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-secondary"><span>Apply Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <button type="button" class="btn-share">
                            <i class="fa-light fa-share-nodes"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="carousel-cell">
                <div class="card-jobs">
                    <div class="card-badge">New</div>
                    <h3 class="job-title">Travel Nurse/Registered Nurse (RN) – MedSurg/Telemetry</h3>
                    <ul class="job-info">
                        <li>
                            <img src="/job-location-icon.svg" alt="">
                            <span><strong>Location:</strong> Charlotte, North Carolina, USA</span>
                        </li>
                        <li>
                            <img src="/job-Specialty-icon.svg" alt="">
                            <span><strong>Specialty:</strong> Health Care</span>
                        </li>
                        <li>
                            <img src="/job-type-icon.svg" alt="">
                            <span><strong>Type:</strong> Contract</span>
                        </li>
                        <li>
                            <img src="/job-Schedule-icon.svg" alt="">
                            <span><strong>Schedule:</strong> Lorem ipsum</span>
                        </li>
                        <li>
                            <img src="/job-time-icon.svg" alt="">
                            <span><strong>Time:</strong> Lorem ipsum</span>
                        </li>
                    </ul>
                    <div class="card-actions">
                        <a href="#" class="btn btn-primary"><span>Read More</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-secondary"><span>Apply Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <button type="button" class="btn-share">
                            <i class="fa-light fa-share-nodes"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="carousel-cell">
                <div class="card-jobs">
                    <div class="card-badge">New</div>
                    <h3 class="job-title">Travel Nurse/Registered Nurse (RN) – MedSurg/Telemetry</h3>
                    <ul class="job-info">
                        <li>
                            <i class="fa-light fa-location-dot"></i>
                            <span><strong>Location:</strong> Charlotte, North Carolina, USA</span>
                        </li>
                        <li>
                            <i class="fa-light fa-stethoscope"></i>
                            <span><strong>Specialty:</strong> Health Care</span>
                        </li>
                        <li>
                            <i class="fa-light fa-file-contract"></i>
                            <span><strong>Type:</strong> Contract</span>
                        </li>
                        <li>
                            <i class="fa-light fa-calendar"></i>
                            <span><strong>Schedule:</strong> Lorem ipsum</span>
                        </li>
                        <li>
                            <i class="fa-light fa-clock"></i>
                            <span><strong>Time:</strong> Lorem ipsum</span>
                        </li>
                    </ul>
                    <div class="card-actions">
                        <a href="#" class="btn btn-primary"><span>Read More</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-secondary"><span>Apply Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <button type="button" class="btn-share">
                            <i class="fa-light fa-share-nodes"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="carousel-cell">
                <div class="card-jobs">
                    <div class="card-badge">New</div>
                    <h3 class="job-title">Travel Nurse/Registered Nurse (RN) – MedSurg/Telemetry</h3>
                    <ul class="job-info">
                        <li>
                            <i class="fa-light fa-location-dot"></i>
                            <span><strong>Location:</strong> Charlotte, North Carolina, USA</span>
                        </li>
                        <li>
                            <i class="fa-light fa-stethoscope"></i>
                            <span><strong>Specialty:</strong> Health Care</span>
                        </li>
                        <li>
                            <i class="fa-light fa-file-contract"></i>
                            <span><strong>Type:</strong> Contract</span>
                        </li>
                        <li>
                            <i class="fa-light fa-calendar"></i>
                            <span><strong>Schedule:</strong> Lorem ipsum</span>
                        </li>
                        <li>
                            <i class="fa-light fa-clock"></i>
                            <span><strong>Time:</strong> Lorem ipsum</span>
                        </li>
                    </ul>
                    <div class="card-actions">
                        <a href="#" class="btn btn-primary"><span>Read More</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-secondary"><span>Apply Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <button type="button" class="btn-share">
                            <i class="fa-light fa-share-nodes"></i>
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="button-wrap">
        <div class="container">
            <a href="#" class="btn btn-primary"><span>Browse All Jobs</span> <i class="fa-regular fa-arrow-right"></i></a>
        </div>
    </div>
</section>
<!-- jobs-sections end -->



<!-- blogs-sections start -->
<section class="blogs-section space-y">
    <div class="section-header mb-5m text-center" data-aos="fade-up">
        <div class="container">
            <div class="inner-wrap">
                <h2>Healthcare Tips, Tricks, And Insights</h2>
                <a href="#" class="cta-link"><span>View All News</span> <i class="fa-light fa-arrow-right-long"></i></a>
                <div class="slider-nav d-flex gap-4">
                    <button class="blog-prev" aria-label="Previous">
                        <i class="fa-regular fa-arrow-left"></i>
                    </button>
                    <button class="blog-next" aria-label="Next">
                        <i class="fa-regular fa-arrow-right"></i>
                    </button>
                </div>
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="blogs-slider" data-aos="fade-up" data-aos-delay="100">
            <div class="blog-item">
                <div class="card-blog">
                    <div class="card-media">
                        <img src="/home-blog-image-1.webp" alt="Blog Image">
                    </div>
                    <div class="card-body">
                        <div class="card-category">Lorem Ipsum</div>
                        <h3 class="card-title">
                            <a href="#">Lorem Ipsum Dolor</a>
                        </h3>
                        <p class="card-text">How do you create compelling presentations that wow your colleagues and impress your managers?</p>
                    </div>
                </div>
            </div>
            <div class="blog-item">
                <div class="card-blog">
                    <div class="card-media">
                        <img src="/home-blog-image-2.webp" alt="Blog Image">
                    </div>
                    <div class="card-body">
                        <div class="card-category">Lorem Ipsum</div>
                        <h3 class="card-title">
                            <a href="#">Lorem Ipsum Dolor</a>
                        </h3>
                        <p class="card-text">How do you create compelling presentations that wow your colleagues and impress your managers?</p>
                    </div>
                </div>
            </div>
            <div class="blog-item">
                <div class="card-blog">
                    <div class="card-media">
                        <img src="/home-blog-image-3.webp" alt="Blog Image">
                    </div>
                    <div class="card-body">
                        <div class="card-category">Lorem Ipsum</div>
                        <h3 class="card-title">
                            <a href="#">Lorem Ipsum Dolor</a>
                        </h3>
                        <p class="card-text">How do you create compelling presentations that wow your colleagues and impress your managers?</p>
                    </div>
                </div>
            </div>
            <div class="blog-item">
                <div class="card-blog">
                    <div class="card-media">
                        <img src="/home-blog-image-4.webp" alt="Blog Image">
                    </div>
                    <div class="card-body">
                        <div class="card-category">Lorem Ipsum</div>
                        <h3 class="card-title">
                            <a href="#">Lorem Ipsum Dolor</a>
                        </h3>
                        <p class="card-text">How do you create compelling presentations that wow your colleagues and impress your managers?</p>
                    </div>
                </div>
            </div>
            <!-- <div class="blog-item">
                <div class="card-blog">
                    <div class="card-media">
                        <img src="/home-blog-image-4.webp" alt="Blog Image">
                    </div>
                    <div class="card-body">
                        <div class="card-category">Lorem Ipsum</div>
                        <h3 class="card-title">
                            <a href="#">Lorem Ipsum Dolor</a>
                        </h3>
                        <p class="card-text">How do you create compelling presentations that wow your colleagues and impress your managers?</p>
                    </div>
                </div>
            </div> -->
        </div>
    </div>
    <div class="button-wrap">
        <div class="container">
            <a href="#" class="btn btn-primary"><span>Browse All Jobs</span> <i class="fa-regular fa-arrow-right"></i></a>
        </div>
    </div>
</section>
<!-- blogs-sections end -->

<section class="recruiters-section space-y" style="background-image: url('/Recruiters-bg-image-scaled.png')">
    <div class="container">
        <div class="section-header mb-5m text-center" data-aos="fade-up">
            <div class="inner-wrap">
                <p><strong>For Recruiters or Candidates</strong></p>
                <h2>Start Your Journey</h2>
            </div>
        </div>
        
        <div class="row g-4">
            <div class="col-12 col-md-6" data-aos="fade-up" data-aos-delay="100">
                <div class="card-recruiters h-100 text-center">
                    <div class="profile-image mx-auto mb-4">
                        <img src="/Find-Work-image.png" alt="Find Work Profile" class="rounded-circle">
                    </div>
                    <h3>Find Work</h3>
                    <p class="mb-4">Yorem ipsum dolor sit amet, consectetur adipiscing elit dolor sit amet.</p>
                    <div class="d-flex gap-3 justify-content-center">
                        <a href="#" class="btn btn-primary"><span>Browse Jobs</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-primary"><span>Register Now</span> <i class="fa-regular fa-arrow-right"></i></a>
                    </div>
                </div>
            </div>
            
            <div class="col-12 col-md-6" data-aos="fade-up" data-aos-delay="200">
                <div class="card-recruiters h-100 text-center">
                    <div class="profile-image mx-auto mb-4">
                        <img src="/Find-Staff-image.png" alt="Find Staff Profile" class="rounded-circle">
                    </div>
                    <h3>Find Staff</h3>
                    <p class="mb-4">Yorem ipsum dolor sit amet, consectetur adipiscing elit dolor sit amet.</p>
                    <div class="d-flex gap-3 justify-content-center">
                        <a href="#" class="btn btn-primary"><span>Contact Us</span> <i class="fa-regular fa-arrow-right"></i></a>
                        <a href="#" class="btn btn-primary"><span>Request Staff</span> <i class="fa-regular fa-arrow-right"></i></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<?php get_footer(); ?>