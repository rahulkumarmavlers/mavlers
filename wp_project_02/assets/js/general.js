...
$(document).ready(function(){
        ...
        $triggerNext.on('click', function() {
            ...
            if ($currentSection.length) {
                ...
                if ($nextSection.length) {
                    ...
                    }, 800, 'linear'); // 800ms for smooth scroll animation
                }
            }
        });
    }
    ...
    const stickyThreshold = 40; // Pixels scrolled before adding class
...
// Function to set equal heights for card-core-goals h3 elements
function setEqualHeights(elm) {
    // Check if screen width is less than 767px (mobile)
    if ($(window).width() < 767) {
...
// Initialize AOS after sliders are built
...
$(document).ready(function() {
    ...
    // Set equal heights for card-core-goals h3
    setEqualHeights(".card-core-goals h3");
    
    // Initialize AOS after a short delay to ensure sliders are built
    initializeAOS();
    // Refresh AOS after slider initialization
    AOS.refresh();

    // Debounced resize handler for equal heights and AOS refresh
    $(window).on('resize', debounce(function() {
        setEqualHeights(".card-core-goals h3");
        AOS.refresh();
    }, 150));

    // Job Sidebar Filter Accordion
    $('.job-sidebar-filter .filter-item strong').on('click', function() {
        ...
        $('.job-sidebar-filter .filter-item').not($parent).removeClass('active')
            .find('.filter-content').slideUp(300);
    });
});
